        ...
        (fun _ ->
            ...
            while // Middle for loop
                ...
                (fun _ ->
                    ...
                    while
                        #(fun h -> live h numoProblems /\ live h jTwiddle /\ live h jFirst /\ live h a /\ live h w)
                        #(fun _ h -> live h numoProblems /\ live h jTwiddle /\ live h jFirst /\ live h a /\ live h w)
                        (fun _ -> jBuf.(size 0) <. jFinish)
                        (fun _ ->
                            let j:size_t = jBuf.(size 0) in
                            let jNumo = j +. numoProblems.(size 0) in
                            let ajNumo:elem = a.(jNumo) in
                            let temp:elem = barr_reduce (reduce (elem_to_int64 (w *^ ajNumo))) in
                            let aj:elem = a.(j) in
                            a.(jNumo) <- barr_reduce (aj +^ ((to_elem 2) *^ params_q -^ temp));
                            a.(j) <- barr_reduce (temp +^ aj);
                ...
                (fun _ ->
                    ...
                    while
                        ...
                        (fun _ ->
                            ...
                            a.(jNumo) <- barr_reduce (reduce (elem_to_int64 (wj *^ (temp +^ ((to_elem 2) *^ params_q -^ ajNumo)))));
			    jBuf.(size 0) <- j +. (size 1)
                        );
                    jFirst.(size 0) <- jBuf.(size 0) +. numoProblems.(size 0)
                );
...
let poly_add_correct result x y =
    ...
    for 0ul params_n
        ...
        (fun i ->
            let tki:elem = result.(i) in
            let mask = (params_q -^ tki) >>^ (size elem_n) -. (size 1) in
            result.(i) <- tki -^ (params_q &^ mask)
        );

    pop_frame()

// No correction in this parameter set for subtraction.
let poly_sub_correct = poly_sub

// No reduction in this parameter set for subtraction.
let poly_sub_reduce = poly_sub
