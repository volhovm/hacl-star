...
let smallfelem_mul out small1 small2 =
  let small10 = small1.(0) in
  let small11 = small1.(1) in
  let small12 = small1.(2) in
  let small13 = small1.(3) in
  let small20 = small2.(0) in
  let small21 = small2.(1) in
  let small22 = small2.(2) in
  let small23 = small2.(3) in
...
let felem_mul out in1 in2 =
	alloc 8 (u64(0)) [BufItem in1; BufItem in2] [BufItem out] 
		(fun h0 _ h1 -> True) (
			fun tmp -> 
  let small1 = sub tmp 0 4 in 
  let small2 = sub tmp 4 4 in 
...
let felem_small_mul out small1 in2 =
  	alloc 4 (u64(0)) [BufItem small1; BufItem in2] [BufItem out] 
		(fun h0 _ h1 -> True) (
			fun tmp -> 
	let small2 = sub tmp 0 4 in 		
	felem_shrink small2 in2;
	smallfelem_mul out small1 small2
)
