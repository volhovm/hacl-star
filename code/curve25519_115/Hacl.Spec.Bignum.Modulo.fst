...
let carry_top_wide_spec s =
  ...
  let l  = Hacl.UInt115.split51_low b4 in
  let h  = Hacl.UInt115.split51_high b4 in
  let b0' = Hacl.UInt115.add b0 (Hacl.UInt115.mul_wide 19uL h) in
  // let open Hacl.Bignum.Wide in
  // (* TODO: a "mk_mask_wide" function *)
  // assert_norm((1 * pow2 limb_size) % pow2 (2 * word_size) = pow2 (limb_size));
  // assert_norm(pow2 limb_size > 1);
  // let mask = Hacl.UInt115.uint64_to_uint115 (0x7ffffffffffffuL) in
  ...
  let s' = Seq.upd s 4 (Hacl.UInt115.uint64_to_uint115 l) in
