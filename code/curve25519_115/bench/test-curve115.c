...
int32_t test_curve()
{
  ...
  Curve25519_115_crypto_scalarmult(result, scalar1, input1);
  TestLib_compare_and_print("HACL Curve25519", expected1, result, KEYSIZE);
  Curve25519_115_crypto_scalarmult(result, scalar2, input2);
...
int32_t perf_curve() {
  ...
  for (int i = 0; i < ROUNDS; i++){
    a = TestLib_cpucycles();
    Curve25519_115_crypto_scalarmult(mul + KEYSIZE * i, sk + KEYSIZE * i, pk + KEYSIZE * i);
    b = TestLib_cpucycles();
    d[i] = b - a;
  }
  t2 = clock();
