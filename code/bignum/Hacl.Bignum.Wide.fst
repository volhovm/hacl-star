...
(* Shift operators *)
inline_for_extraction val shift_right: a:wide -> s:FStar.UInt32.t{FStar.UInt32.v s < wide_n} -> Tot (c:wide{w c = (w a / (pow2 (FStar.UInt32.v s)))})
inline_for_extraction let shift_right a s = wide_shift_right a s

inline_for_extraction val shift_left: a:wide -> s:FStar.UInt32.t{FStar.UInt32.v s < wide_n} -> Tot (c:wide{w c = (w a * pow2 (FStar.UInt32.v s)) % pow2 wide_n})
