...
inline_for_extraction let limb_zero = uint32_to_sint32 0ul
inline_for_extraction let limb_one  = uint32_to_sint32 1ul
...
inline_for_extraction let limb_to_byte x = assert_norm(pow2 8 = 256); sint32_to_sint8 x
inline_for_extraction let byte_to_limb x = sint8_to_sint32 x

(* Wide limb operators *)
inline_for_extraction let wide_n = 2 * word_size

inline_for_extraction val w: wide -> GTot (FStar.UInt.uint_t wide_n)
inline_for_extraction let w x = Hacl.UInt64.v x
...
inline_for_extraction val uint32_to_limb: x:FStar.UInt32.t{FStar.UInt32.v x < pow2 word_size} ->
  Tot (y:limb{v y = FStar.UInt32.v x})

inline_for_extraction let wide_zero = uint64_to_sint64 (0uL)
inline_for_extraction let wide_one  = uint64_to_sint64 (1uL)

(* Addition primitives *)
inline_for_extraction let wide_add a b = Hacl.UInt64.add a b
inline_for_extraction let wide_add_mod a b = Hacl.UInt64.add_mod a b
(* Subtraction primitives *)
inline_for_extraction let wide_sub a b = Hacl.UInt64.sub a b
inline_for_extraction let wide_sub_mod a b = Hacl.UInt64.sub_mod a b

(* Bitwise operators *)
inline_for_extraction let wide_logand a b = Hacl.UInt64.logand a b
inline_for_extraction let wide_logxor a b = Hacl.UInt64.logxor a b
inline_for_extraction let wide_logor  a b = Hacl.UInt64.logor  a b
inline_for_extraction let wide_lognot a   = Hacl.UInt64.lognot a

(* Shift operators *)
inline_for_extraction let wide_shift_right a s = Hacl.UInt64.shift_right a s
inline_for_extraction let wide_shift_left  a s = Hacl.UInt64.shift_left a s

inline_for_extraction let wide_eq_mask a b  = Hacl.UInt64.eq_mask a b
inline_for_extraction let wide_gte_mask a b = Hacl.UInt64.gte_mask a b
inline_for_extraction let wide_gt_mask a b  = Hacl.UInt64.gt_mask a b
inline_for_extraction let wide_lt_mask a b  = Hacl.UInt64.lt_mask a b
inline_for_extraction let wide_lte_mask a b = Hacl.UInt64.lte_mask a b
...
inline_for_extraction let mul_wide x y =
  ...
  assert(w (limb_to_wide x) * w (limb_to_wide y) <= pow2 64 - 1);
  assert(w (limb_to_wide x) * w (limb_to_wide y) >= 0);
  assert(UInt.size (w (limb_to_wide x) * w (limb_to_wide y)) 64);
