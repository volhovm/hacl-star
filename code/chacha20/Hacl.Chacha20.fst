...
let chacha20_encrypt len out text key n ctr = admit (); Hacl.Impl.Chacha20.chacha20_encrypt len out text key n ctr
...
val chacha20_decrypt: len:size_t -> out:lbuffer uint8 len -> text:lbuffer uint8 len -> key:lbuffer uint8 32ul -> n:lbuffer uint8 12ul -> ctr:size_t -> ST unit
		  (requires (fun h -> live h key /\ live h n /\ live h text /\ live h out))
		  (ensures (fun h0 _ h1 -> modifies (loc out) h0 h1))
let chacha20_decrypt len out text key n ctr = admit (); Hacl.Impl.Chacha20.chacha20_decrypt len out text key n ctr
