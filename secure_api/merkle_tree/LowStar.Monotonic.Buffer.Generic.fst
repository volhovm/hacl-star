...
let size2nat (sz:size_t): Tot nat =
  match sz with
  | SZ32 x -> U32.v x
  | SZ64 x -> U64.v x

inline_for_extraction      
let index2nat (i:index_t) = 
  match i with
  | IDX32 x -> U32.v x
  | IDX64 x -> U64.v x
...
inline_for_extraction
unfold
let same_index_width (i:index_t) (j:index_t): Tot bool =
  match i, j with
  | IDX32 _, IDX32 _ 
  | IDX64 _, IDX64 _ -> true
  | _ -> false

unfold
let index_add_pre (i:index_t) (j:index_t): Tot Type0 =
  let sum = index2nat i + index2nat j in
  match i, j with 
  | IDX32 x, IDX32 y -> UInt.size sum 32
  | IDX64 x, IDX64 y -> UInt.size sum 64
  | _ -> False
  
inline_for_extraction
let index_add (i:index_t) (j:index_t): Pure (r:index_t)
  (requires (index_add_pre i j))
  (ensures (fun r -> index2nat i + index2nat j = index2nat r))
  = match i, j with
    | IDX32 x, IDX32 y -> IDX32 (U32.add x y)
    | IDX64 x, IDX64 y -> IDX64 (U64.add x y)
    | IDX32 x, IDX64 y -> IDX64 (U64.add (FStar.Int.Cast.uint32_to_uint64 x) y)
    | IDX64 x, IDX32 y -> IDX64 (U64.add x (FStar.Int.Cast.uint32_to_uint64 y))

let size_sub_pre (s:size_t) (i:index_t): Tot bool =
  let diff = size2nat s - index2nat i in
  match s, i with 
  | SZ32 x, IDX32 y -> UInt.fits diff 32
  | SZ64 x, IDX64 y -> UInt.fits diff 64
  | _ -> false
  
inline_for_extraction
let size_sub (s:size_t) (i:index_t): Pure (r:size_t)
  (requires (size_sub_pre s i))
  (ensures (fun r -> size2nat s - index2nat i = size2nat r))
  = match s, i with
    | SZ32 x, IDX32 y -> SZ32 (U32.sub x y)
    | SZ64 x, IDX64 y -> SZ64 (U64.sub x y)


private let srel_to_lsrel (#a:Type0) (len:nat) (pre:srel a) :Preorder.preorder (Seq.lseq a len) = fun s1 s2 -> pre s1 s2
...
noeq type mbuffer (a:Type0) (size_t:Type0) (rrel:srel a) (rel:srel a) :Type0 =
  | Null
  | Buffer:    
    max_length:size_t ->
    content:HST.mreference (Seq.lseq a (size2nat max_length)) (srel_to_lsrel (size2nat max_length) rrel) ->
    idx:size_t ->
    length:size_t{size2nat idx + size2nat length <= size2nat max_length} ->
    compatible:squash (compatible_sub_preorder (size2nat max_length) rrel
                                               (size2nat idx) (size2nat idx + size2nat length) rel) ->  //proof of compatibility
    mbuffer a size_t rrel rel
...
let index_zero (#a:Type0) (#rrel #rel:srel a) (b:mbuffer a rrel rel): Tot index_t =
  match b with
  | Buffer (SZ32 x) _ _ _ _ -> IDX32 0ul
  | _ -> IDX64 0UL

let size_zero (#a:Type0) (#rrel #rel:srel a) (b:mbuffer a rrel rel): Tot size_t =
  match b with
  | Buffer (SZ32 x) _ _ _ _ -> SZ32 0ul
  | _ -> SZ64 0UL

let size_one (#a:Type0) (#rrel #rel:srel a) (b:mbuffer a rrel rel): Tot size_t =
  match b with
  | Buffer (SZ32 x) _ _ _ _ -> SZ32 1ul
  | _ -> SZ64 1UL

let same_size_width (#a:Type0) (#rrel1 #rel1:srel a) (#rrel2 #rel2:srel a) (b1:mbuffer a rrel1 rel1) (b2:mbuffer a rrel2 rel2): Tot Type0 =
  match b1, b2 with
  | Buffer (SZ32 _) _ _ _ _, Buffer (SZ32 _) _ _ _ _
  | Buffer (SZ64 _) _ _ _ _, Buffer (SZ64 _) _ _ _ _ -> True
  | _ -> False
...
let as_seq #_ #_ #_ h b =
  ...
  | Buffer max_len content idx len _ ->
    Seq.slice (HS.sel h content) (index2nat idx) (index2nat idx + size2nat len)
...
let mgsub #a #rrel #rel sub_rel b i len =
  ...
  | Buffer max_len content idx length () ->
    lemma_seq_sub_compatibility_is_transitive (size2nat max_len) rrel
                                              (index2nat idx) (index2nat idx + size2nat length) rel
		         	              (index2nat i) (index2nat i + size2nat len) sub_rel;
    match max_len, idx, i with
    | SZ32 _, IDX32 x, IDX32 y ->
      let ni = IDX32 (U32.add x y) in
      Buffer max_len content ni len ()
    | SZ64 _, IDX64 x, IDX64 y ->
      let ni = IDX64 (U64.add x y) in
      Buffer max_len content ni len ()
    | _ -> Null

let live_gsub #a #rrel #rel h b i sz sub_rel = 
  assert (index2nat i + size2nat sz <= length b /\ compatible_sub b i sz sub_rel);
  let m = mgsub sub_rel b i sz in
  assert (same_size_width b m);
  assert (live h b ==> live h m);
  assert (live h m ==> live h b);
  ()

let gsub_is_null #_ #_ #_ _ _ _ _ = ()

let len_gsub #a #rrel #rel b i len' sub_rel = 
  assert (index2nat i + size2nat len' <= length b /\ compatible_sub b i len' sub_rel);
  assert (len (mgsub sub_rel b i len') == len');
  ()
...
let gsub_gsub #_ #_ #rel b i1 len1 sub_rel1 i2 len2 sub_rel2 =
  lemma_seq_sub_compatibility_is_transitive (length b) rel (index2nat i1) (index2nat i1 + size2nat len1) sub_rel1
                                            (index2nat i2) (index2nat i2 + size2nat len2) sub_rel2
#pop-options
...
let as_seq_gsub #_ #_ #_ h b i len _ =
  ...
  | Buffer _ content idx len0 _ ->
    Seq.slice_slice (HS.sel h content) (index2nat idx) (index2nat idx + size2nat len0) (index2nat i) (index2nat i + size2nat len)
...
let ubuffer (region: HS.rid) (addr: nat) : Tot Type0 = Ghost.erased (ubuffer' region addr)
...
let msub #a #rrel #rel sub_rel b i len =
  ...
  | Buffer max_len content i0 len0 () ->
    lemma_seq_sub_compatibility_is_transitive (size2nat max_len) rrel (index2nat i0) (index2nat i0 + size2nat len0) rel
                                              (index2nat i) (index2nat i + size2nat len) sub_rel;
...
let moffset #a #rrel #rel sub_rel b i =
  ...
  | Buffer max_len content i0 len () ->
    lemma_seq_sub_compatibility_is_transitive (size2nat max_len) rrel (index2nat i0) (index2nat i0 + size2nat len) rel
                                              (index2nat i) (index2nat i + U32.v (U32.sub len i)) sub_rel;
...
let g_upd_seq #_ #_ #_ b s h =
  if Seq.length s = 0 then h
    ...
    HS.upd h (Buffer?.content b) (Seq.replace_subseq s0 (index2nat idx) (index2nat idx + size2nat length) s)
...
let upd' #_ #_ #_ b i v =
  ...
  let s0 = !content in
  let sb0 = Seq.slice s0 (index2nat idx) (index2nat idx + size2nat len) in
  content := Seq.replace_subseq s0 (index2nat idx) (index2nat idx + size2nat len) (Seq.upd sb0 (index2nat i) v)
...
private let alloc_heap_common (#a:Type0) (#rrel:srel a)
  (r:HST.erid) (init:a) (len:U32.t{size2nat len > 0}) (mm:bool)
  :HST.ST (lmbuffer a rrel rrel (size2nat len))
          (requires (fun _      -> True))
          (ensures (fun h0 b h1 -> alloc_post_mem_common b h0 h1 (Seq.create (size2nat len) init) /\
	                        frameOf b == r /\
                                ...
                                Buffer?.idx b == 0ul /\
                                Buffer?.length b == Buffer?.max_length b))
  = let s = Seq.create (size2nat len) init in
    lemma_seq_sub_compatilibity_is_reflexive (size2nat len) rrel;
    let content: HST.mreference (Seq.lseq a (size2nat len)) (srel_to_lsrel (size2nat len) rrel) =
...
let malloca #a #rrel init len =
  lemma_seq_sub_compatilibity_is_reflexive (size2nat len) rrel;
  let content: HST.mreference (Seq.lseq a (size2nat len)) (srel_to_lsrel (size2nat len) rrel) =
    HST.salloc (Seq.create (size2nat len) init)
  in
  let b = Buffer len content 0ul len () in
  b
...
let blit #a #rrel1 #rrel2 #rel1 #rel2 src idx_src dst idx_dst len =
  ...
  if len = 0ul then ()
    ...
    let s1 = Seq.slice s_full1 (index2nat idx1) (index2nat idx1 + size2nat length1) in
    let s2 = Seq.slice s_full2 (index2nat idx2) (index2nat idx2 + size2nat length2) in
    let s_sub_src = Seq.slice s1 (index2nat idx_src) (index2nat idx_src + size2nat len) in
    let s2' = Seq.replace_subseq s2 (index2nat idx_dst) (index2nat idx_dst + size2nat len) s_sub_src in
    let s_full2' = Seq.replace_subseq s_full2 (index2nat idx2) (index2nat idx2 + size2nat length2) s2' in
    assert (Seq.equal (Seq.slice s2' (index2nat idx_dst) (index2nat idx_dst + size2nat len)) s_sub_src);
    assert (Seq.equal (Seq.slice s2' 0 (index2nat idx_dst)) (Seq.slice s2 0 (index2nat idx_dst)));
    assert (Seq.equal (Seq.slice s2' (index2nat idx_dst + size2nat len) (length dst))
                      (Seq.slice s2 (index2nat idx_dst + size2nat len) (length dst)));
    content2 := s_full2';
...
: HST.Stack unit
  ...
  (ensures  (fun h0 _ h1 ->
    ...
    live h1 b /\
    Seq.slice (as_seq h1 b) 0 (size2nat len) `Seq.equal` Seq.create (size2nat len) z /\
    Seq.slice (as_seq h1 b) (size2nat len) (length b) `Seq.equal` Seq.slice (as_seq h0 b) (size2nat len) (length b)
  ))
= let open HST in
  if len = 0ul then ()
  else begin
    ...
    let s = Seq.slice s_full (index2nat idx) (index2nat idx + size2nat length) in
    let s_src = Seq.create (size2nat len) z in
    let s' = Seq.replace_subseq s 0 (size2nat len) s_src in
    let s_full' = Seq.replace_subseq s_full (index2nat idx) (index2nat idx + size2nat len) s_src in
    assert (s_full' `Seq.equal` Seq.replace_subseq s_full (index2nat idx) (index2nat idx + size2nat length) s');
    content := s_full';
    let h' = HST.get () in
