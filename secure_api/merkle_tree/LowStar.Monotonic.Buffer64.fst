...
let ubuffer_of_buffer' (#a:Type0) (#rrel:srel a) (#rel:srel a) (b:mbuffer a rrel rel)
  ...
  = if Null? b
    then
      ...
      Ghost.hide ({
        b_max_length = U64.v (Buffer?.max_length b);
        b_offset = U64.v (Buffer?.idx b);
        b_length = U64.v (Buffer?.length b);
...
let ubuffer_includes_ubuffer_preserved #r #a larger smaller h1 h2 =
  ubuffer_preserved_intro smaller h1 h2 (fun t' b' ->
    ...
    then ()
      ...
      let idx = U64.uint_to_t (G.reveal larger).b_offset in
      let len = U64.uint_to_t (G.reveal larger).b_length in
      let larger' = Buffer max_len content idx len in
      assert (b' == gsub larger' (U64.sub idx' idx) len');
