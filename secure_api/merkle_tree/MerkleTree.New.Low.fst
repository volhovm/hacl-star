...
/// 1. `hash` is regional (in `hreg`)

open MerkleTree.New.Low.Hash


/// 2. `hash_vec` is regional (vector of hashes; in `hvreg`)

open MerkleTree.New.Low.HashVec


/// 3. `hash_vv`: regional vectors of vectors of hashes (in `hvvreg`)

open MerkleTree.New.Low.HashVecVec


/// The Merkle tree implementation begins here.

/// Utilities for hashes
...
//    (ensures (fun h0 _ h1 ->
//      // memory safety
//      B.modifies (B.loc_region_only false (B.frameOf dst)) h0 h1 /\
...
// let hash_2 #a #rg src1 src2 dst =
//   let hh0 = HST.get () in
//   HST.push_frame ();
//   // KreMLin can't extract `EHS.blockLen EHS.SHA256` (= 64ul)
//   let cb = B32.alloca #a 0uy 64ul in  
//   B32.blit (B.to_specific #32 #a src1) 0ul cb 0ul (UInt32.uint_to_t hash_size);
//   B32.blit (B.to_specific #32 #a src2) 0ul cb 32ul (UInt32.uint_to_t hash_size);
...
//                   (B32.as_seq hh1 cb));
//   let dst32 = B32.alloca 0uy 64ul in
//   EHS.finish #(Ghost.hide EHS.SHA256) st dst32;
//   let hh3 = HST.get () in
//   assert (S.equal (B32.as_seq hh3 dst32)
...
//   HST.pop_frame ();
//   let hh4 = HST.get () in
//   assert (S.equal (B32.as_seq hh4 dst32)
//                   (High.hash_2
//                     (Rgl?.r_repr hreg hh0 src1)
//                     (Rgl?.r_repr hreg hh0 src2)));
//   // TODO: copy dst32 to dst
...
// inline_for_extraction val merkle_tree_size_lg: index_t
// inline_for_extraction let merkle_tree_size_lg = UInt.to_uint_t 64 64
...
// type mt_p = B.pointer #64 merkle_tree

// // The maximum number of elements in the tree is (2^n - 1).
// val mt_not_full: HS.mem -> mt_p -> GTot bool
// let mt_not_full h mt =
//   MT?.j (B.get h mt 0) < UInt.to_uint_t 64 (UInt.max_int UInt64.n)

// /// (Memory) Safety

// val offset_of: i:index_t -> Tot index_t
// let offset_of i =
//   if i % (UInt.to_uint_t 64 2) = (UInt.zero 64) then i else i - (UInt.to_uint_t 64 1)
...
// // element.
// val mt_safe_elts:
//   h:HS.mem -> lv:index_t{lv <= merkle_tree_size_lg} ->
//   hs:hash_vv{V.size_of hs = merkle_tree_size_lg} ->
//   i:index_t -> j:index_t{j >= i} ->
//   GTot Type0 (decreases (64 - lv))
...
//        mt_safe_elts h (lv + (UInt.one 64)) hs (i / (UInt.to_uint_t 64 2)) (j / (UInt.to_uint_t 64 2)))
