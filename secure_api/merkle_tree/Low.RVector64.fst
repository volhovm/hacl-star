...
val rv_elems_inv:
  ...
  i:V.index_t -> j:V.index_t{i <= j && j <= V.size_of rv} ->
  GTot Type0
let rv_elems_inv #a #rg h rv i j =
  rs_elems_inv rg h (V.as_seq h rv) (V.index2nat i) (V.index2nat j) 
...
let elems_inv #a #rg h rv =
  rv_elems_inv h rv V.sz0 (V.size_of rv)
...
let rv_elems_reg #a #rg h rv i j =
  rs_elems_reg rg (V.as_seq h rv) (V.frameOf rv) (V.index2nat i) (V.index2nat j)
...
let elems_reg #a #rg h rv =
  rv_elems_reg h rv V.sz0 (V.size_of rv)
...
let rv_elems_inv_live_region #a #rg h rv i j =
  rs_elems_inv_live_region rg h (V.as_seq h rv) (V.index2nat i) (V.index2nat j)
...
let rv_loc_elems #a #rg h rv i j =
  rs_loc_elems rg (V.as_seq h rv) (V.index2nat i) (V.index2nat j)
  ...
  if i + 1 = j then ()
       ...
       assert (rs_loc_elems rg rs i j ==
	      B.loc_union (B.loc_union
			...
			(rs_loc_elem rg rs (j - 1)));
       B.loc_union_assoc (rs_loc_elem rg rs i)
		       (rs_loc_elems rg rs (i + 1) (j - 1))
		       (rs_loc_elem rg rs (j - 1)))
...
val rv_loc_elems_included:
  #a:Type0 -> #rg:regional a ->
  h:HS.mem -> rv:rvector rg ->
  i:index_t -> j:index_t{i <= j && j <= V.size_of rv} ->
...
let rv_loc_elems_included #a #rg h rv i j =
  rs_loc_elems_included rg (V.as_seq h rv) (V.frameOf rv) (V.index2nat i) (V.index2nat j)
...
val rv_loc_elems_disj:
  ...
  i:index_t -> j:index_t{i <= j && j <= V.size_of rv} -> 
  k1:index_t{i <= k1} ->
  k2:index_t{k1 <= k2 && k2 <= j} ->
  l1:index_t{i <= l1} ->
  l2:index_t{l1 <= l2 && l2 <= j} ->
...
let rv_loc_elems_disj #a #rg h rv i j k1 k2 l1 l2 =
  rs_loc_elems_disj rg (V.as_seq h rv) (V.frameOf rv)
    (V.index2nat i) (V.index2nat j) (V.index2nat k1) (V.index2nat k2) (V.index2nat l1) (V.index2nat l2)
...
val rs_loc_elems_parent_disj:
  ...
  i:nat -> j:nat{i <= j && j <= S.length rs} -> 
  Lemma (requires (rs_elems_reg rg rs prid i j))
	(ensures (B.loc_disjoint (rs_loc_elems rg rs i j)
			       (B.loc_region_only false prid)))
	(decreases j)
...
val rv_loc_elems_parent_disj:
  ...
  i:index_t -> j:index_t{i <= j && j <= V.size_of rv} -> 
  Lemma (requires (rv_elems_reg h rv i j))
	(ensures (B.loc_disjoint (rv_loc_elems h rv i j)
			       (B.loc_region_only false (V.frameOf rv))))
let rv_loc_elems_parent_disj #a #rg h rv i j =
  rs_loc_elems_parent_disj rg (V.as_seq h rv) (V.frameOf rv) (V.index2nat i) (V.index2nat j)
...
val rs_loc_elems_each_disj:
  ...
  Lemma (requires (V.forall_seq rs i j
	...
	(ensures (B.loc_disjoint (rs_loc_elems rg rs i j)
			       (B.loc_all_regions_from false drid)))
	(decreases j)
...
let rv_loc_elems_each_disj #a #rg h rv i j drid =
  rs_loc_elems_each_disj rg (V.as_seq h rv) drid (V.index2nat i) (V.index2nat j)
...
val rv_loc_elems_preserved:
  ...
  Lemma (requires (V.live h0 rv /\
		  V.loc_disjoint p (V.loc_vector_within rv i j) /\
		  modifies p h0 h1))
	(ensures (rv_loc_elems h0 rv i j ==
		 rv_loc_elems h1 rv i j))
	(decreases (V.index2nat j))
let rec rv_loc_elems_preserved #a #rg rv i j p h0 h1 =
  if i = j then ()
  else (V.loc_vector_within_includes rv i j (j - sz1) j;
       V.get_preserved rv (j - sz1) p h0 h1;
       assert (V.get h0 rv (j - sz1) == V.get h1 rv (j - sz1));
       V.loc_vector_within_includes rv i j i (j - sz1);
       rv_loc_elems_preserved rv i (j - sz1) p h0 h1)

val rs_elems_inv_preserved:
  #a:Type0 -> rg:regional a -> rs:S.seq a -> 
  i:nat -> j:nat{i <= j && j <= S.length rs} ->
  p:B.loc -> h0:HS.mem -> h1:HS.mem ->
...
let rv_elems_inv_preserved #a #rg rv i j p h0 h1 =
  rs_elems_inv_preserved rg (V.as_seq h0 rv) (V.index2nat i) (V.index2nat j) p h0 h1
...
val rv_inv_preserved_:
  ...
  Lemma (requires (rv_inv h0 rv /\
		  ...
		  loc_disjoint p (rv_loc_elems h0 rv V.sz0 (V.size_of rv)) /\
		  modifies p h0 h1))
	(ensures (rv_inv h1 rv))
let rv_inv_preserved_ #a #rg rv p h0 h1 =
  rv_elems_inv_preserved #a #rg rv V.sz0 (V.size_of rv) p h0 h1
...
let rv_inv_preserved #a #rg rv p h0 h1 =
  ...
  rv_loc_elems_included h0 rv V.sz0 (V.size_of rv);
  assert (loc_includes (loc_rvector rv) (rv_loc_elems h0 rv V.sz0 (V.size_of rv)));
  rv_inv_preserved_ rv p h0 h1

val rv_inv_preserved_int:
  #a:Type0 -> #rg:regional a -> rv:rvector rg -> 
  i:index_t{i < V.size_of rv} ->
  h0:HS.mem -> h1:HS.mem ->
  Lemma (requires (rv_inv h0 rv /\
		  modifies (B.loc_all_regions_from false 
...
let rv_inv_preserved_int #a #rg rv i h0 h1 =
  rs_loc_elems_elem_disj
    ...
    0 (V.index2nat (V.size_of rv)) 0 (V.index2nat i) (V.index2nat i);
  rs_elems_inv_preserved
    rg (V.as_seq h0 rv) 0 (V.index2nat i)
    (B.loc_all_regions_from false
      (Rgl?.region_of rg (V.get h1 rv i)))
    h0 h1;
  rs_loc_elems_elem_disj
    rg (V.as_seq h0 rv) (V.frameOf rv)
    0 (V.index2nat (V.size_of rv)) 
    (V.index2nat i + 1) (V.index2nat (V.size_of rv)) (V.index2nat i);
  rs_elems_inv_preserved
    rg (V.as_seq h0 rv) (V.index2nat i + 1) (V.index2nat (V.size_of rv))
    (B.loc_all_regions_from false
      (Rgl?.region_of rg (V.get h1 rv i)))
    h0 h1
...
val as_seq_sub:
  ...
  GTot (s:S.seq (Rgl?.repr rg){S.length s = V.index2nat j - V.index2nat i})
       (decreases (V.index2nat j))
let rec as_seq_sub #a #rg h rv i j =
  as_seq_seq rg h (V.as_seq h rv) (V.index2nat i) (V.index2nat j)

val as_seq:
  #a:Type0 -> #rg:regional a -> 
  h:HS.mem -> rv:rvector rg{rv_inv h rv} ->
  GTot (s:S.seq (Rgl?.repr rg){S.length s = V.index2nat (V.size_of rv)})
let rec as_seq #a #rg h rv =
  as_seq_sub h rv V.sz0 (V.size_of rv)

val as_seq_sub_as_seq:
  #a:Type0 -> #rg:regional a -> 
  h:HS.mem -> rv:rvector rg{rv_inv h rv} ->
  Lemma (S.equal (as_seq_sub h rv V.sz0 (V.size_of rv))
		 (as_seq h rv))
	[SMTPat (as_seq_sub h rv V.sz0 (V.size_of rv))]
...
let as_seq_sub_preserved #a #rg rv i j p h0 h1 =
  as_seq_seq_preserved rg (V.as_seq h0 rv) (V.index2nat i) (V.index2nat j) p h0 h1
...
let as_seq_preserved_ #a #rg rv p h0 h1 =
  as_seq_sub_preserved rv V.sz0 (V.size_of rv) p h0 h1
...
val create_empty:
  ...
  HST.ST (rvector rg)
    ...
    (ensures (fun h0 bv h1 -> h0 == h1 /\ V.size_of bv = V.sz0))
let create_empty #a rg =
  V.create_empty a

val create_:
  #a:Type0 -> #rg:regional a -> rv:rvector rg ->
  cidx:index_t{cidx <= V.size_of rv} -> 
  HST.ST unit
    (requires (fun h0 -> rv_itself_inv h0 rv))
    (ensures (fun h0 _ h1 ->
      modifies (V.loc_vector_within rv V.sz0 cidx) h0 h1 /\
      rv_itself_inv h1 rv /\
      rv_elems_inv h1 rv V.sz0 cidx /\
      rv_elems_reg h1 rv V.sz0 cidx /\
      S.equal (as_seq_sub h1 rv V.sz0 cidx)
      	      (S.create (V.index2nat cidx) (Ghost.reveal (Rgl?.irepr rg))) /\
      // the loop invariant for this function
      V.forall_ h1 rv V.sz0 cidx
	(fun r -> MHS.fresh_region (Rgl?.region_of rg r) h0 h1 /\
		  Rgl?.r_init_p rg r) /\
      Set.subset (Map.domain (MHS.get_hmap h0))
		 (Map.domain (MHS.get_hmap h1))))
    (decreases (V.index2nat cidx))
...
let rec create_ #a #rg rv cidx =
  ...
  if cidx = V.sz0 then ()
       ...
       let hh1 = HST.get () in
       V.assign rv (cidx - sz1) v;

       let hh2 = HST.get () in
       V.loc_vector_within_included rv (cidx - sz1) cidx;
       Rgl?.r_sep
	 rg (V.get hh2 rv (cidx - sz1))
	 (V.loc_vector_within rv (cidx - sz1) cidx)
	 hh1 hh2;
       create_ rv (cidx - sz1);

       let hh3 = HST.get () in
       V.loc_vector_within_included rv V.sz0 (cidx - sz1);
       Rgl?.r_sep
	 rg (V.get hh3 rv (cidx - sz1))
	 (V.loc_vector_within rv V.sz0 (cidx - sz1))
	 hh2 hh3;
       V.forall2_extend hh3 rv V.sz0 (cidx - sz1)
       	 (fun r1 r2 -> HH.disjoint (Rgl?.region_of rg r1)
       				   (Rgl?.region_of rg r2));
       V.loc_vector_within_union_rev rv V.sz0 cidx)

val create_rid:
  #a:Type0 -> rg:regional a ->
  len:size_t{len > V.sz0} -> rid:erid ->
  HST.ST (rvector rg)
    (requires (fun h0 -> true))
    (ensures (fun h0 rv h1 ->
      ...
      S.equal (as_seq h1 rv) 
	      (S.create (V.index2nat len) (Ghost.reveal (Rgl?.irepr rg)))))
let create_rid #a rg len rid =
  let vec = V.create_rid len (Rgl?.dummy rg) rid in
  create_ #a #rg vec len;
  V.loc_vector_within_included vec V.sz0 len;
  vec
...
val insert:
  ...
  HST.ST (rvector rg)
    ...
    (ensures (fun h0 irv h1 ->
      V.size_of irv = V.size_of rv + sz1 /\
...
let insert #a #rg rv v =
  ...
  rs_loc_elems_parent_disj
    rg (V.as_seq hh0 rv) (V.frameOf rv) 0 (V.index2nat (V.size_of rv));
  rs_elems_inv_preserved
    rg (V.as_seq hh0 rv) 0 (V.index2nat (V.size_of rv))
    (B.loc_region_only false (V.frameOf rv))
    hh0 hh1;
  Rgl?.r_sep rg v
    (B.loc_region_only false (V.frameOf rv))
  ...
  assert (S.equal (V.as_seq hh0 rv)
  		  (S.slice (V.as_seq hh1 irv) 0 (V.index2nat (V.size_of rv))));
  as_seq_seq_preserved
    rg (V.as_seq hh0 rv)
    0 (V.index2nat (V.size_of rv))
    (B.loc_region_only false (V.frameOf rv)) hh0 hh1;
  as_seq_seq_slice 
    rg hh1 (V.as_seq hh1 irv) 0 (V.index2nat (V.size_of irv))
    0 (V.index2nat (V.size_of rv));
  irv
...
let insert_copy #a #rg cp rv v =
  let hh0 = HST.get () in
  rv_elems_inv_live_region hh0 rv V.sz0 (V.size_of rv);
  ...
  let hh2 = HST.get () in
  rv_loc_elems_each_disj hh2 rv V.sz0 (V.size_of rv) nrid;
  rv_inv_preserved_ rv (B.loc_all_regions_from false nrid) hh1 hh2;
  as_seq_preserved_ rv (B.loc_all_regions_from false nrid) hh1 hh2;
  insert rv nv

val assign:
  #a:Type0 -> #rg:regional a -> rv:rvector rg -> 
  i:index_t{i < V.size_of rv} -> v:a ->
  HST.ST unit
    (requires (fun h0 ->
      // rv_inv h0 rv /\
      rv_itself_inv h0 rv /\
      rv_elems_inv h0 rv V.sz0 i /\
      rv_elems_inv h0 rv (i + sz1) (V.size_of rv) /\
      elems_reg h0 rv /\
      
      V.forall_ h0 rv V.sz0 i
	(fun b -> HH.disjoint (Rgl?.region_of rg b)
			      (Rgl?.region_of rg v)) /\
      V.forall_ h0 rv (i + sz1) (V.size_of rv)
      	(fun b -> HH.disjoint (Rgl?.region_of rg b)
      			      (Rgl?.region_of rg v)) /\
      Rgl?.r_inv rg h0 v /\
      HH.extends (Rgl?.region_of rg v) (V.frameOf rv)))
    (ensures (fun h0 _ h1 -> 
      modifies (V.loc_vector_within rv i (i + sz1)) h0 h1 /\
      rv_inv h1 rv /\
      V.get h1 rv i == v /\
      S.equal (as_seq h1 rv)
	      (S.append
		(as_seq_sub h0 rv V.sz0 i)
		(S.cons (Rgl?.r_repr rg h0 v)
		  (as_seq_sub h0 rv (i + sz1) (V.size_of rv))))))
let assign #a #rg rv i v =
  ...
  // Safety
  rs_loc_elems_parent_disj
    rg (V.as_seq hh0 rv) (V.frameOf rv) 0 (V.index2nat i);
  rs_loc_elems_parent_disj
    rg (V.as_seq hh0 rv) (V.frameOf rv) (V.index2nat i + 1) (V.index2nat (V.size_of rv));
  rs_elems_inv_preserved
    rg (V.as_seq hh0 rv) 0 (V.index2nat i)
    (V.loc_vector rv)
    hh0 hh1;
  rs_elems_inv_preserved
    rg (V.as_seq hh0 rv) (V.index2nat i + 1) (V.index2nat (V.size_of rv))
    (V.loc_vector rv)
    hh0 hh1;
  ...
  // Correctness
  rs_loc_elems_parent_disj
    rg (V.as_seq hh1 rv) (V.frameOf rv) 0 (V.index2nat (V.size_of rv));
  as_seq_seq_preserved
    rg (V.as_seq hh1 rv)
    0 (V.index2nat (V.size_of rv))
...
private val r_sep_forall:
  ...
  Lemma (requires (loc_disjoint (B.loc_all_regions_from
...
val assign_copy:
  ...
  HST.ST unit
    ...
    (ensures (fun h0 _ h1 -> 
      ...
      rv_inv h1 rv /\
      S.equal (as_seq h1 rv)
      	      (S.upd (as_seq h0 rv) (V.index2nat i) (Rgl?.r_repr rg h0 v))))
let assign_copy #a #rg cp rv i v =
  ...
  forall_intro
    (move_requires
  	...
  	0 (V.index2nat (V.size_of rv))
  	(V.index2nat i)));
  assert (forall (k:nat{k <> V.index2nat i && k < V.index2nat (V.size_of rv)}).
  	   loc_disjoint (rs_loc_elem rg (V.as_seq hh0 rv) k)
  			(rs_loc_elem rg (V.as_seq hh0 rv) (V.index2nat i)));
  forall_intro
    (move_requires
      (r_sep_forall
  	rg (rs_loc_elem rg (V.as_seq hh0 rv) (V.index2nat i))
  	hh0 hh1));
  assert (forall (k:nat{k <> V.index2nat i && k < V.index2nat (V.size_of rv)}).
  	   loc_disjoint (rs_loc_elem rg (V.as_seq hh0 rv) k)
  			(rs_loc_elem rg (V.as_seq hh0 rv) (V.index2nat i)) ==>
  ...
  assert (forall (k:nat{k <> V.index2nat i && k < V.index2nat (V.size_of rv)}).
  	   S.index (as_seq_seq rg hh1 (V.as_seq hh1 rv)
  			       0 (V.index2nat (V.size_of rv))) k ==
  	   S.index (as_seq_seq rg hh0 (V.as_seq hh0 rv)
  			       0 (V.index2nat (V.size_of rv))) k)

val free_elems:
  #a:Type0 -> #rg:regional a -> rv:rvector rg -> 
  idx:index_t{idx < V.size_of rv} ->
  HST.ST unit
    (requires (fun h0 -> 
      V.live h0 rv /\
        rv_elems_inv h0 rv V.sz0 (idx + sz1) /\
        rv_elems_reg h0 rv V.sz0 (idx + sz1)))
    (ensures (fun h0 _ h1 ->
      idx < V.size_of rv ==> modifies (rv_loc_elems h0 rv V.sz0 (idx + sz1)) h0 h1))
let rec free_elems #a #rg rv idx =
  let hh0 = HST.get () in
    ...
    rs_loc_elems_elem_disj
      rg (V.as_seq hh0 rv) (V.frameOf rv)
      0 (V.index2nat idx + 1) 0 (V.index2nat idx) (V.index2nat idx);
    rv_elems_inv_preserved 
      rv V.sz0 idx (rs_loc_elem rg (V.as_seq hh0 rv) (V.index2nat idx)) hh0 hh1;

    if idx <> V.sz0 then
      free_elems rv (idx - sz1)

val flush:
  #a:Type0 -> #rg:regional a ->
  rv:rvector rg -> i:index_t{i <= V.size_of rv} ->
  HST.ST (rvector rg)
    ...
    (ensures (fun h0 frv h1 ->
      ...
      S.equal (as_seq h1 frv)
      	      (S.slice (as_seq h0 rv) (V.index2nat i) (V.index2nat (V.size_of rv)))))
#reset-options "--z3rlimit 40"
let flush #a #rg rv i =
  let hh0 = HST.get () in
  (if i = V.sz0 then () else free_elems rv (i - sz1));
  rv_loc_elems_included hh0 rv V.sz0 i;

  let hh1 = HST.get () in
  assert (modifies (rs_loc_elems rg (V.as_seq hh0 rv) 0 (V.index2nat i)) hh0 hh1);
  let frv = V.flush rv (Rgl?.dummy rg) i in

  let hh2 = HST.get () in
  assert (modifies (B.loc_region_only false (V.frameOf rv)) hh1 hh2);

  // Safety
  rs_loc_elems_disj
    rg (V.as_seq hh0 rv) (V.frameOf rv) 0 (V.index2nat (V.size_of rv))
    0 (V.index2nat i) (V.index2nat i) (V.index2nat (V.size_of rv));
  rs_loc_elems_parent_disj
    rg (V.as_seq hh0 rv) (V.frameOf rv)
    (V.index2nat i) (V.index2nat (V.size_of rv));
  rs_elems_inv_preserved
    rg (V.as_seq hh0 rv) (V.index2nat i) (V.index2nat (V.size_of rv))
    (loc_union (rs_loc_elems rg (V.as_seq hh0 rv) 0 (V.index2nat i))
	       (B.loc_region_only false (V.frameOf rv)))
    hh0 hh2;
  ...
  as_seq_seq_preserved
    ...
    hh0 hh2;
  as_seq_seq_slice
    rg hh0 (V.as_seq hh0 rv) 0 (V.index2nat (V.size_of rv))
    (V.index2nat i) (V.index2nat (V.size_of rv));
  assert (S.equal (S.slice (as_seq hh0 rv) (V.index2nat i) (V.index2nat (V.size_of rv)))
		  (as_seq_seq rg hh2 (V.as_seq hh0 rv)
		    (V.index2nat i) (V.index2nat (V.size_of rv))));
  as_seq_seq_eq
    rg hh2 (V.as_seq hh0 rv) (V.as_seq hh2 frv)
    (V.index2nat i) (V.index2nat (V.size_of rv)) 0 (V.index2nat (V.size_of frv));
  assert (S.equal (as_seq_seq rg hh2 (V.as_seq hh2 frv)
		    0 (V.index2nat (V.size_of frv)))
		  (as_seq_seq rg hh2 (V.as_seq hh0 rv)
		    (V.index2nat i) (V.index2nat (V.size_of rv))));
  assert (S.equal (S.slice (as_seq hh0 rv) (V.index2nat i) (V.index2nat (V.size_of rv)))
		  (as_seq hh2 frv));
...
let free #a #rg rv =
  let hh0 = HST.get () in
  (if V.size_of rv = V.sz0 then () 
  else free_elems rv (V.size_of rv - sz1));
  let hh1 = HST.get () in
  rv_loc_elems_included hh0 rv V.sz0 (V.size_of rv);
  V.free rv
