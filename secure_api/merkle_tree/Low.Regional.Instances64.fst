...
val buffer_r_sep:
  #a:Type -> len:size_t{len > sz0} ->
  v:B.buffer a -> p:B.loc -> h0:HS.mem -> h1:HS.mem ->
...
let buffer_r_sep #a len v p h0 h1 =
  assert (B.loc_includes (B.loc_all_regions_from false (buffer_region_of v))
		       (B.loc_buffer v));
...
val buffer_r_init:
  #a:Type -> ia:a -> len:size_t{len > sz0} -> r:erid ->
  HST.ST (b:B.buffer a)
...
val buffer_r_free:
  ...
  HST.ST unit
    ...
    (ensures (fun h0 _ h1 ->
      B.modifies (B.loc_all_regions_from false (buffer_region_of v)) h0 h1))
...
val vector_r_sep:
  #a:Type -> #rg:regional a ->
  v:rvector rg -> p:B.loc -> h0:HS.mem -> h1:HS.mem ->
  Lemma (requires (vector_r_inv h0 v /\
		  B.loc_disjoint 
		    (B.loc_all_regions_from false (vector_region_of v))
		    p /\
		  B.modifies p h0 h1))
...
val vector_r_free:
  ...
  HST.ST unit
    ...
    (ensures (fun h0 _ h1 ->
      B.modifies (B.loc_all_regions_from false (vector_region_of v)) h0 h1))
